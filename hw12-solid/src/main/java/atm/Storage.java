...
public class Storage {
    ...
    public Storage(EnumMap<Denomination, Integer> state) {
        this.atmState = state;
    }
    ...
    public void removeBanknotes(List<Banknote> banknotes) {
        for (Banknote banknote : banknotes) {
            if (!atmState.containsKey(banknote.getDenomination()) || atmState.get(banknote.getDenomination()) == 0) {
                throw new AtmException("В банкомате закончились купюры данного номинала, все сломалось, обратитесь к специалисту, который писал алгоритм");
            }
